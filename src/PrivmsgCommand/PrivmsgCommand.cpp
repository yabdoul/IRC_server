...
void PrivmsgCommand::execute(Client& sender, std::map<std::string, std::string>& params)
{
    ...
    if (params.find("message") == params.end()) {
        sender.addMsg(serverResponseFactory::getResp(412, sender));  // ERR_NOTEXTTOSEND
        return;
    }
    
    // Check target type
    if (params.find("target_type") == params.end()) {
        sender.addMsg(serverResponseFactory::getResp(461, sender));  // ERR_NEEDMOREPARAMS
        return;
    }
    ...
    if (targetType == "channel") {
        ...
        try {
            ...
            if (!targetChannel.isUserInChannel(sender)) {
                sender.addMsg(serverResponseFactory::getResp(404, sender));  // ERR_CANNOTSENDTOCHAN
                return;
            }
            ...
            sender.addMsg(serverResponseFactory::getResp(403, sender));  // ERR_NOSUCHCHANNEL
        }
        ...
        try {
            ...
            sender.addMsg(serverResponseFactory::getResp(401, sender));  // ERR_NOSUCHNICK
        }
